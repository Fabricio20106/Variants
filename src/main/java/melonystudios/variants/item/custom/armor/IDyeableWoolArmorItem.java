...
public interface IDyeableWoolArmorItem extends IDyeableArmorItem {
    ...
    static ItemStack dyeArmor(ItemStack stack, List<Integer> colors) {
        ...
        int i = 0;
        int j = 0;
        IDyeableWoolArmorItem dyeableWoolArmorItem = null;
        Item item = stack.getItem();

        if (item instanceof IDyeableWoolArmorItem) {
            dyeableWoolArmorItem = (IDyeableWoolArmorItem) item;
            emptyStack = stack.copy();
            emptyStack.setCount(1);
            if (dyeableWoolArmorItem.hasCustomColor(stack)) {
                int color = dyeableWoolArmorItem.getColor(emptyStack);
        ...
        if (dyeableWoolArmorItem == null) {
            ...
            int j2 = (red << 8) + green;
            j2 = (j2 << 8) + blue;
            dyeableWoolArmorItem.setColor(emptyStack, j2);
            return emptyStack;
        }
    }
}
