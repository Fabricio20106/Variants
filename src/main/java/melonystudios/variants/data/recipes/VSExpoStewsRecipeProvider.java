...
public class VSExpoStewsRecipeProvider extends RecipeProvider {
    ...
    private void makeMushroomStew(Consumer<IFinishedRecipe> consumer, ItemStack stewStack, StewBehavior behavior) {
        for (BowlType type : DEFAULT_BOWLS) {
            ...
            ExponentialStewRecipeBuilder.shapeless(resultStack).requires(Items.BROWN_MUSHROOM).requires(Items.RED_MUSHROOM).requires(type.getBowlStack().getItem()).group("mushroom_stew").unlockedBy("has_bowl", has(type.getBowlStack()
    ...
    private void makeBeetrootSoup(Consumer<IFinishedRecipe> consumer, ItemStack stewStack, StewBehavior behavior) {
        for (BowlType type : DEFAULT_BOWLS) {
            ItemStack resultStack = getResultStack(stewStack, type, behavior);
            ExponentialStewRecipeBuilder.shapeless(resultStack).requires(Items.BEETROOT).requires(Items.BEETROOT).requires(Items.BEETROOT).requires(Items.BEETROOT).requires(Items.BEETROOT).requires(Items.BEETROOT).requires(type.getBowlStack().getItem())
    ...
    private void makeRabbitStew(Consumer<IFinishedRecipe> consumer, ItemStack stewStack, StewBehavior behavior) {
        for (BowlType type : DEFAULT_BOWLS) {
            ItemStack resultStack = getResultStack(stewStack, type, behavior);
            ExponentialStewRecipeBuilder.shapeless(resultStack).requires(Items.BAKED_POTATO).requires(Items.COOKED_RABBIT).requires(Items.CARROT).requires(Ingredient.of(Items.RED_MUSHROOM, Items.BROWN_MUSHROOM)).requires(type.getBowlStack().getItem())
    ...
    private void makeFungiStew(Consumer<IFinishedRecipe> consumer, ItemStack stewStack, StewBehavior behavior) {
        for (BowlType type : DEFAULT_BOWLS) {
            ItemStack resultStack = getResultStack(stewStack, type, behavior);
            ExponentialStewRecipeBuilder.shapeless(resultStack).requires(Items.CRIMSON_FUNGUS).requires(Items.WARPED_FUNGUS).requires(type.getBowlStack().getItem())
    ...
    private void makeEndFungiStew(Consumer<IFinishedRecipe> consumer, ItemStack stewStack, StewBehavior behavior) {
        for (BowlType type : DEFAULT_BOWLS) {
            ItemStack resultStack = getResultStack(stewStack, type, behavior);
            ExponentialStewRecipeBuilder.shapeless(resultStack).requires(VSItems.ENDER_FUNGUS.get()).requires(VSItems.ENDER_FUNGUS.get()).requires(type.getBowlStack().getItem())
    ...
    private void makeWaterBowl(Consumer<IFinishedRecipe> consumer, ItemStack stewStack, StewBehavior behavior) {
        for (BowlType type : DEFAULT_BOWLS) {
            ItemStack resultStack = getResultStack(stewStack, type, behavior);
            ExponentialStewRecipeBuilder.shapeless(resultStack).requires(Items.POTION).requires(Items.POTION).requires(type.getBowlStack().getItem())
    ...
    private void makeLavaBowl(Consumer<IFinishedRecipe> consumer, ItemStack stewStack, StewBehavior behavior) {
        for (BowlType type : DEFAULT_BOWLS) {
            ItemStack resultStack = getResultStack(stewStack, type, behavior);
            ExponentialStewRecipeBuilder.shapeless(resultStack).requires(VSItems.LAVA_BOTTLE.get()).requires(VSItems.LAVA_BOTTLE.get()).requires(type.getBowlStack().getItem())
    ...
    private void makeSoulLavaBowl(Consumer<IFinishedRecipe> consumer, ItemStack stewStack, StewBehavior behavior) {
        for (BowlType type : DEFAULT_BOWLS) {
            ItemStack resultStack = getResultStack(stewStack, type, behavior);
            ExponentialStewRecipeBuilder.shapeless(resultStack).requires(VSItems.SOUL_LAVA_BOTTLE.get()).requires(VSItems.SOUL_LAVA_BOTTLE.get()).requires(type.getBowlStack().getItem())
    ...
    private void makeMilkBowl(Consumer<IFinishedRecipe> consumer, ItemStack stewStack, StewBehavior behavior) {
        for (BowlType type : DEFAULT_BOWLS) {
            ItemStack resultStack = getResultStack(stewStack, type, behavior);
            ExponentialStewRecipeBuilder.shapeless(resultStack).requires(VSItems.MILK_BOTTLE.get()).requires(VSItems.MILK_BOTTLE.get()).requires(type.getBowlStack().getItem())
    ...
    private void makePowderSnowBowl(Consumer<IFinishedRecipe> consumer, ItemStack stewStack, StewBehavior behavior) {
        for (BowlType type : DEFAULT_BOWLS) {
            ItemStack resultStack = getResultStack(stewStack, type, behavior);
            ExponentialStewRecipeBuilder.shapeless(resultStack).requires(VSItems.POWDER_SNOW_BOTTLE.get()).requires(VSItems.POWDER_SNOW_BOTTLE.get()).requires(type.getBowlStack().getItem())
