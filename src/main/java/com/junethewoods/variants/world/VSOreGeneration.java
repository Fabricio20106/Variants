...
public class VSOreGeneration {
    ...
    public static void generateQuartzOre(final BiomeLoadingEvent event) {
        if (!(event.getCategory().equals(Biome.Category.NETHER) || event.getCategory().equals(Biome.Category.THEEND))) {
            generateOre(event.getGeneration(), OreFeatureConfig.FillerBlockType.NATURAL_STONE, VSBlocks.QUARTZ_ORE.get().defaultBlockState(), 6, 10, 117, 4);
        }
    }
    ...
    public static void generateEndQuartzOre(final BiomeLoadingEvent event) {
        if (event.getCategory().equals(Biome.Category.THEEND)) {
            generateOre(event.getGeneration(), BASE_STONE_END, VSBlocks.END_QUARTZ_ORE.get().defaultBlockState(), 6, 10, 117, 4);
        }
    }
    ...
    public static void generateOres() {
        MinecraftForge.EVENT_BUS.addListener(EventPriority.HIGH, VSOreGeneration::generateQuartzOre);
        MinecraftForge.EVENT_BUS.addListener(EventPriority.HIGH, VSOreGeneration::generateEndQuartzOre);
    }
}
