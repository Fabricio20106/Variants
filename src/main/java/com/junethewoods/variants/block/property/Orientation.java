package com.junethewoods.variants.block.property;
...
public enum Orientation implements IStringSerializable {
    ...
    private static final Int2ObjectMap<Orientation> LOOKUP_TOP_FRONT = Util.make(new Int2ObjectOpenHashMap<>(values().length), (hashMap) -> {
        Orientation[] orientations = values();
        for (Orientation orientation : orientations) hashMap.put(lookupKey(orientation.front, orientation.top), orientation);
    });
    ...
    Orientation(String name, Direction top, Direction front) {
        this.name = name;
        this.top = top;
        this.front = front;
    }
    ...
    public static Orientation fromFrontAndTop(Direction front, Direction top) {
