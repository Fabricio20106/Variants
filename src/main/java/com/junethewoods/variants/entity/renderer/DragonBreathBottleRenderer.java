...
@OnlyIn(Dist.CLIENT)
public class DragonBreathBottleRenderer extends EntityRenderer<DragonBreathBottleEntity> {
    private final RenderType CUTOUT_NO_CULL = RenderType.entityCutoutNoCull(Variants.resourceLoc("textures/item/soul_charge.png"));

    public DragonBreathBottleRenderer(EntityRendererManager manager) {
        super(manager);
    }

    public void render(DragonBreathBottleEntity breathBottle, float f, float partialTicks, MatrixStack stack, IRenderTypeBuffer buffer, int packedLight) {
        stack.pushPose();
        ...
        stack.popPose();
        super.render(breathBottle, f, partialTicks, stack, buffer, packedLight);
    }
    ...
    @Override
    public ResourceLocation getTextureLocation(DragonBreathBottleEntity breathBottle) {
        return Variants.resourceLoc("textures/item/splash_dragon_breath.png");
    }
}
